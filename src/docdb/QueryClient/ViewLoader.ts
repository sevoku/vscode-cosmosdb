...
import { EditorUserQuery, QueryEditorCommand, QueryEditorMessage } from "./messageContract";
...
export default class ViewLoader {
    ...
    public sendCommand(command: QueryEditorMessage) {
        ...
        void this._panel!.webview.postMessage(command);
    }
    ...
    private getWebviewContent() {
        ...
        let cssUrl: string | undefined = undefined;

        const isProduction = true;

        if (isProduction) {
            scriptUrl = this._panel?.webview
                .asWebviewUri(vscode.Uri.file(path.join(this.options.extensionPath, "src", "docdb", "query-editor", "dist", "assets", jsFile)))
                .toString();
            cssUrl = this._panel?.webview
                .asWebviewUri(vscode.Uri.file(path.join(this.options.extensionPath, "src", "docdb", "query-editor", "dist", "assets", cssFile)))
                .toString();
        } else {
